...
function updated_belief_particles=MCL(priorbelief_particles,uk,zk,simpar)
    ...
    updated_belief_particles = sample_motion_model(uk,priorbelief_particles,simpar);
	predict_particles = updated_belief_particles; % change this value for the correct one
    particle_weights = measurement_model(zk,updated_belief_particles,simpar);

    %resample using particle weights
    beta = 0;
    ind = floor(rand(1)*(length(particle_weights)-1))+1;
    for i=1:length(particle_weights)

        beta = beta + 2*rand(1)*max(particle_weights);
        %resampling wheel
        while (particle_weights(ind) < beta)
            beta = beta - particle_weights(ind);
            ind = mod(ind + 1,length(particle_weights));
            if ind == 0
                ind = 1;
            end
        end
        
        newParticles(i) = updated_belief_particles(ind);
    
    end
    
    updated_belief_particles = newParticles;
    
    %pdfs for the animation
    measurement_model_particles = pdf('Normal',[1:simpar.domain],simpar.door_locations(1),simpar.door_stdev,simpar.domain);
    measurement_model_particles = measurement_model_particles + pdf('Normal',[1:simpar.domain],simpar.door_locations(2),simpar.door_stdev,simpar.domain);
    measurement_model_particles = measurement_model_particles + pdf('Normal',[1:simpar.domain],simpar.door_locations(3),simpar.door_stdev,simpar.domain);
    measurement_model_particles = measurement_model_particles./3;
	...
	DrawParticles(5,'Weighted Particles',predict_particles,particle_weights*50, simpar);
...
function predict_particles=sample_motion_model(uk,prior_belief_particles,simpar)
    predict_particles = mod(prior_belief_particles+ones(1,simpar.numberOfParticles)*uk+simpar.odometry_stdev.*randn(1,simpar.numberOfParticles),simpar.domain);
...
function particle_weights=measurement_model(zk,pdf_particles,simpar)
    ...
    predict_particles = pdf(pd1, pdf_particles);
    predict_particles = predict_particles + pdf(pd2, pdf_particles);
    predict_particles = predict_particles + pdf(pd3, pdf_particles);
    predict_particles = predict_particles./3;
    
    if zk == 0 %P( zk=0 | xk ) = 1-P( zk=1 | xk )
        predict_particles = ones(1,simpar.numberOfParticles) - predict_particles;
    end

    particle_weights = predict_particles/sum(predict_particles);
